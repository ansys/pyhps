...
#   timestamp: 2023-11-15T12:39:13+00:00
...
class KubernetesKedaBackend(BaseModel):
    plugin_name: Literal["kubernetes"] = Field(..., Literal=True, title="Plugin Name")
...
class KubernetesResourceScaling(BaseModel):
    plugin_name: Literal["kubernetes_resource_scaling"] = Field(
        ..., Literral=True, title="Plugin Name"
    )
    target_resource_kind: Optional[str] = Field(
        "job",
        description="kubernetes resource kind rep scaler should scale, should be one of job, deployment, statefulset",
...
class LocalBackend(BaseModel):
    plugin_name: Literal["local"] = Field(..., Literral=True, title="Plugin Name")
    debug: Optional[bool] = Field(
        False, description="Enable additional debugging of the backend", title="Debug"
    )
    working_dir: Optional[str] = Field(
...
class MaxAvailableResourceScaling(BaseModel):
    plugin_name: Literal["max_available_resource_scaling"] = Field(
        ..., Literral=True, title="Plugin Name"
    )
...
class MockupBackend(BaseModel):
    plugin_name: Literal["mockup"] = Field(..., Literral=True, title="Plugin Name")
    debug: Optional[bool] = Field(
        False, description="Enable additional debugging of the backend", title="Debug"
    )
...
class ProcessLauncherProcessRunner(BaseModel):
    plugin_name: Literal["process_launcher_module"] = Field(..., Literral=True, title="Plugin Name")
...
class RestLauncherProcessRunner(BaseModel):
    plugin_name: Literal["process_launcher_service"] = Field(
        ..., Literral=True, title="Plugin Name"
    )
    default_user: Optional[str] = Field(
        None, description="The user to use when none is specified", title="Default User"
...
class ServiceUserProcessRunner(BaseModel):
    plugin_name: Literal["service_user_module"] = Field(..., Literral=True, title="Plugin Name")
...
class OrchestrationInterfacesBackend(BaseModel):
    plugin_name: Literal["orchestration_interfaces"] = Field(
        ..., Literral=True, title="Plugin Name"
    )
    debug: Optional[bool] = Field(
        False, description="Enable additional debugging of the backend", title="Debug"
...
class ComputeResourceSet(BaseModel):
    ...
    available_resources: Optional[Resources] = Field(
        default_factory=lambda: Resources.parse_obj({"custom": {}}),
