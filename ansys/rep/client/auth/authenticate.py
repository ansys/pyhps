...
import requests
from ansys.rep.client import ClientError

from ..exceptions import raise_for_status

log = logging.getLogger(__name__)

def get_oidc_response(server_url:str, *, grant_type:str, client_id:str, client_secret:str=None, username:str=None, password:str=None, realm:str="rep", scope='openid', refresh_token=None, timeout=10):
    ...
    if r.status_code != 200:
        raise RuntimeError(f"Failed to retrieve access token for client {client_id} from {token_url} using {grant_type} grant, status code {r.status_code}: {r.content.decode()}")

    return r.json()

def authenticate(url:str="https://127.0.0.1:8443/rep", realm:str="rep", 
