...
class EvaluatorSchema(ObjectSchema):
    ...
    host_id = fields.String(
        description="Unique identifier built from hardware information and selected job_definition details of an evaluator."
    )
    ...
    project_server_select = fields.Bool(
        allow_none=True,
        description="Whether the evaluator allows server-driven assignment of projects or uses it's own local settings.",
    )
    ...
    update_time = fields.DateTime(
        allow_none=True,
        load_only=True,
        description="Last time the evaluator updated it's registration details. Used to check which evaluators are alive.",
    )
    ...
    configuration = fields.Dict(
        allow_none=True,
        description="Details of the evaluator configuration, including hardware info and available applications.",
    )
