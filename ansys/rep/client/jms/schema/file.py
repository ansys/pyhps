...
class FileSchema(ObjectSchema):
    ...
    type = fields.String(
        allow_none=True,
        description="Type of the file. This can be any string but using a correct media type for the given resource is advisable.",
    )
    ...
    evaluation_path = fields.String(
        allow_none=True,
        description="Relative path under which the file instance for a design point evaluation will be stored.",
    )
    ...
    collect_interval = fields.Int(
        allow_none=True,
        description="Collect frequency for a file with collect=True. Min value limited by the evaluator's settings. 0/None - let the evaluator decide, other value - interval in seconds",
    )
