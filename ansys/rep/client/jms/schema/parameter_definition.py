...
class ParameterDefinitionBaseSchema(ObjectSchema):
    ...
    quantity_name = fields.String(allow_none = True, description="Name of the quantity the parameter represents, e.g. Length.")
    units = fields.String(allow_none = True, description="Units for the parameter.")
    display_text = fields.String(allow_none = True, description="Text to display as the parameter name.")

    mode = fields.String(load_only=True, description="Indicates whether it's an input or output parameter. Filled server side.")
...
class FloatParameterDefinitionSchema(ParameterDefinitionBaseSchema):
    ...
    step = fields.Float(allow_none = True, description="If provided, allowable values are given by: AllowableValue = lower_limit + n * step, where n is an integer and AllowableValue <= upper_limit.")
    cyclic = fields.Bool(allow_none = True, description="Indicates if the parameter is cyclic.")
    value_list = fields.List(fields.Float(), allow_none = True, description="A list of allowed values, alternative to providing upper and lower limits.")
...
class IntParameterDefinitionSchema(ParameterDefinitionBaseSchema):
    ...
    lower_limit = fields.Integer(allow_none = True, description="Lower bound for the parameter value.")
    upper_limit = fields.Integer(allow_none = True, description="Upper bound for the parameter value.")
...
class StringParameterDefinitionSchema(ParameterDefinitionBaseSchema):
    ...
    value_list = fields.List(fields.String(), allow_none = True, description="A list of allowed values.")
