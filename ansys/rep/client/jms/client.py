...
from .resource.project import (Project, copy_project, create_project,
                               delete_project, get_project, get_projects,
                               update_project, archive_project, restore_project)
from .resource.task_definition_template import (get_task_definition_templates,
...
class Client(object):
    ...
    def __init__(self,
        ...
        *,
        ...
        if access_token:
            ...
            tokens = authenticate(url=auth_url or rep_url, 
                realm=realm,
    ...
    def _auto_refresh_token(self, response, *args, **kwargs):
        ...
        if response.status_code == 401 and self.unauthorized_num_retry < self._unauthorized_max_retry:
            self.unauthorized_num_retry += 1
            self.refresh_access_token()
            response.request.headers.update( {"Authorization" : self.session.headers["Authorization"]} )
