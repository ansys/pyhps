...
class Object(object):
    ...
    class Meta:
        ...
        rest_name=None # String used in REST URI's to access this resource, to be set in derived classes
    ...
    def __repr__(self):
        return "%s(%s)" % (self.__class__.__name__, ",".join(['%s=%r' %(k,getattr(self,k)) for k in self.declared_fields()]))
...
def get_objects(project, obj_type, job_definition=None, as_objects=True, **query_params):
    ...
    for o in objects: o.project=project
    return objects
