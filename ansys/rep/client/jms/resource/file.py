    ...
    for f in files:
        if getattr(f, "src", None) is not None:
            with open(f.src, "rb") as file_content:
                r = project.client.session.post(
                    f"{project.fs_bucket_url}/{f.storage_id}", data=file_content, headers=fs_headers
                )
...
def download_file(file, target_path, progress_handler=None, stream=True):
    ...
    with file.project.client.session.get(download_link, stream=stream) as r, open(
        download_path, "wb"
    ) as f:
