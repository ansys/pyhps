...
def _do_venv(context):
    ...
    subprocess.run(f"{context.system_python_binary} -m venv {context.venv_name}", shell=True, check=True)

    pip_conf_path = os.path.join(context.venv_name, "pip.ini") if sys.platform == "win32" else os.path.join(context.venv_name, "pip.conf")
    ...
    subprocess.run(f"{context.python_binary} -m pip install --upgrade {' '.join(base_modules)}", shell=True, check=True)
...
def _do_documentation(context):
    ...
    subprocess.run(f"{context.python_binary} -m pip install --upgrade {' '.join(docu_modules)}", shell=True, check=True)
    subprocess.run(f"{context.python_binary} prepare_documentation.py", shell=True, check=True)
    subprocess.run(f"{context.python_binary} -m sphinx -b html {docs_directory} {target_directory}", shell=True, check=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("selected_steps", nargs='*', default='all', choices=step_names + ['all'], help = "Steps selected to run")
    parser.add_argument("-n", "--no", dest='disabled_steps', action="append", help="Disable selected steps", default=[], choices=step_names)
