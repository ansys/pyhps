...
def main(client, num_jobs):
    ...
    log.debug("=== Project")
    proj = Project(name="two_bar_truss_problem", display_name="Two-bar Truss Problem", priority=1, active=True)
    ...
    files.append (File( name="input_params",evaluation_path="input_parameters.json",
                        type="text/plain", src=os.path.join(cwd, "input_parameters.json") ) )
    files.append (File( name="pyscript",evaluation_path="evaluate.py",
                        type="text/plain", src=os.path.join(cwd, "evaluate.py") ) )
    ...
    # Input params
    input_params = [
        FloatParameterDefinition(name='height', lower_limit=10, upper_limit=100.0, default=30, units="in" ),
        FloatParameterDefinition(name='diameter', lower_limit=0.2, upper_limit=5, default=3, units="in" ),
        FloatParameterDefinition(name='thickness', lower_limit=0.03, upper_limit=0.6, default=0.15, units="in"),
        FloatParameterDefinition(name='separation_distance', lower_limit=40, upper_limit=150, default=60, units="in" ),
        FloatParameterDefinition(name='young_modulus', lower_limit=1E+6, upper_limit=1E+8, default=3E+7, units="lbs in^-2" ),
        FloatParameterDefinition(name='density', lower_limit=0.1, upper_limit=0.6, default=0.3, units="lbs in^-2" ),
        FloatParameterDefinition(name='load', lower_limit=1E+1, upper_limit=1E+5, default=66E+3, units="lbs" ),
    ]
    input_params = proj.create_parameter_definitions(input_params)

    mappings = [
        ParameterMapping(key_string='"H"', tokenizer=":", parameter_definition_id=input_params[0].id, file_id=input_file.id),
        ParameterMapping(key_string='"d"', tokenizer=":", parameter_definition_id=input_params[1].id, file_id=input_file.id),
        ParameterMapping(key_string='"t"', tokenizer=":", parameter_definition_id=input_params[2].id, file_id=input_file.id),
        ParameterMapping(key_string='"B"', tokenizer=":", parameter_definition_id=input_params[3].id, file_id=input_file.id),
        ParameterMapping(key_string='"E"', tokenizer=":", parameter_definition_id=input_params[4].id, file_id=input_file.id),
        ParameterMapping(key_string='"rho"', tokenizer=":", parameter_definition_id=input_params[5].id, file_id=input_file.id),
        ParameterMapping(key_string='"P"', tokenizer=":", parameter_definition_id=input_params[6].id, file_id=input_file.id),
    ]
    ...
    mappings.extend([
        ParameterMapping(key_string='"weight"', tokenizer=":", parameter_definition_id=output_params[0].id, file_id=result_file.id),
        ParameterMapping(key_string='"stress"', tokenizer=":", parameter_definition_id=output_params[1].id, file_id=result_file.id),
        ParameterMapping(key_string='"buckling_stress"', tokenizer=":", parameter_definition_id=output_params[2].id, file_id=result_file.id),
        ParameterMapping(key_string='"deflection"', tokenizer=":", parameter_definition_id=output_params[3].id, file_id=result_file.id),
    ])
    ...
    for i in range(num_jobs):
        values = { p.name : p.lower_limit + random.random()*(p.upper_limit-p.lower_limit)  for p in params if p.mode == 'input' }
