...
async def get_token_info(authorization_response):
    d = {}
    # token = loop.run_until_complete(client.fetch_token(token_endpoint, authorization_response=authorization_response, redirect_uri=redirect_uri))
    token = await client.fetch_token(token_endpoint, authorization_response=authorization_response, redirect_uri=redirect_uri)
    d["token"] = {}
    ...
    # To retrieve groups an extra mapper needs to be added: https://stackoverflow.com/questions/56362197/keycloak-oidc-retrieve-user-groups-attributes
