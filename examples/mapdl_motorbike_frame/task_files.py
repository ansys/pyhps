...
def main():
    ...
    client = Client(rep_url="https://127.0.0.1/dcs",
                    username="repadmin", password="repadmin")
    ...
    for t, dp in zip(tasks, dps):
        log.debug(f"Task: id={t.id} eval_status={t.eval_status} input_file_ids={t.input_file_ids} ouptut_file_ids={t.output_file_ids} Design point: id={dp.id} eval_status={dp.eval_status}")
        # log.debug(f'{t}')
        # log.debug(f'{dp}')
