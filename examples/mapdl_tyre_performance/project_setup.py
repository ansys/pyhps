...
from ansys.rep.client.jms import (Client, File, FitnessDefinition,
                                  ...
                                  SuccessCriteria, TaskDefinition)
...
def main(client, name, num_jobs):
    ...
    files=[]    
    files.append (File( name="mac",evaluation_path="dcs_tire_performance_simulation.mac",
                        type="text/plain", src=os.path.join(cwd, "dcs_tire_performance_simulation.mac") ) )
    files.append (File( name="geom",evaluation_path="2d_tire_geometry.iges",
                        type="text/plain", src=os.path.join(cwd, "2d_tire_geometry.iges") ) )
    files.append( File( name="results", evaluation_path="tire_performance_results.txt", type="text/plain") )
    ...
    files.append( File( name="mntr", evaluation_path="file.mntr", type="text/plain", collect=True, monitor=True) )    
    ...
    # Input params
    input_params = [
        FloatParameterDefinition(name='camber_angle', display_text="Camber Angle", lower_limit=-8.0, upper_limit=8.0, default=0.0),
        FloatParameterDefinition(name='inflation_pressure', display_text="Inflation Pressure", lower_limit=0.15e06, upper_limit=0.3e06, default=0.24e06),
        FloatParameterDefinition(name='rotational_velocity', display_text="Rotational Velocity", lower_limit=0.0, upper_limit=70.0, default=50.0),
        FloatParameterDefinition(name='translational_velocity', display_text="Translational Velocity", lower_limit=0.0, upper_limit=30.0, default=20.0),
    ]
    input_params = proj.create_parameter_definitions(input_params)

    param_mappings = [
        ParameterMapping(key_string='camber_angle', tokenizer="=", parameter_definition_id=input_params[0].id, file_id=file_ids["mac"]),
        ParameterMapping(key_string='inflation_pressure', tokenizer="=", parameter_definition_id=input_params[1].id, file_id=file_ids["mac"]),
        ParameterMapping(key_string='rotational_velocity', tokenizer="=", parameter_definition_id=input_params[2].id, file_id=file_ids["mac"]),
        ParameterMapping(key_string='translational_velocity', tokenizer="=", parameter_definition_id=input_params[3].id, file_id=file_ids["mac"]),
    ]
    ...
    param_mappings.extend([
        ParameterMapping(key_string="CP Time      (sec)", tokenizer="=", parameter_definition_id=output_params[0].id, file_id=file_ids["out"]),
        ParameterMapping(key_string="Elapsed Time (sec)", tokenizer="=", parameter_definition_id=output_params[1].id, file_id=file_ids["out"]),
    ])
