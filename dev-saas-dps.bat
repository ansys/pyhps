...
REM set BASE_URL=https://localhost:8443/hps
...
REM for /f "delims=" %%b in ('python create_self_signed_token.py --signing_key="D:/ansysDev/signing.key" --account="%ACCOUNT_BURST%" --token=%ANSYS_TOKEN%') do @set TOKEN=%%b
...
python examples/mapdl_motorbike_frame/project_setup.py --name "2003 FILE TESTING JON" -v "2025 R2" --use-exec-script --url "%BASE_URL%" --account="%ACCOUNT_BURST%" --num-jobs=1 --token=%TOKEN%
REM python examples/generic_api/project_setup.py --urls "%BASE_URL%"  --accounts "%ACCOUNT%" --token=%TOKEN%
