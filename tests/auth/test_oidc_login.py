...
def test_load_tokens_prefers_keyring(sample_tokens, tmp_path, monkeypatch):
    """load_tokens() prefers keyring over disk in oidc wrapper."""
...
def test_load_tokens_fallback_to_disk(sample_tokens, tmp_path, monkeypatch):
    """load_tokens() falls back to disk in oidc wrapper."""
    token_file = tmp_path / ".ansys" / "hps_tokens.json"
    monkeypatch.setattr("ansys.hps.client.auth.api.oidc_login.TOKEN_FILE", token_file)
...
def test_load_tokens_uses_env_service_name(monkeypatch):
    """load_tokens uses service name configured via environment variable."""
