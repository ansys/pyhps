...
class TaskFilesTest(REPTestCase):
    ...
    def test_task_files_in_single_task_definition_project(self):
        ...
        ex_dir = os.path.join(cwd, "..", "..", "examples",
        ...
        for i, t in enumerate(tasks):
            ...
            files.append(File(name="mac2", evaluation_path="task_input_file.mac",
                              type="text/plain", src=mac2_fpath))

            # Add an extra task output file
            files.append(File(name="task_result", evaluation_path="task_output_file.txt",
                              type="text/plain", collect=True))
        ...
        def wait_for_evaluation_of_all_jobs(proj):
            ...
            while num_jobs_finished < num_jobs and dt < max_eval_time:
                ...
                num_jobs_finished = len([job for job in jobs if job.eval_status in [
                                                "evaluated", "timeout", "failed"]])
                log.info(
        ...
        def check_evaluated_tasks(proj, tasks):
            for t in tasks:
                ...
                self.assertEqual(
                    len(set(t.input_file_ids).intersection(t.owned_file_ids)), 2)
                input_files = proj.get_files(id=t.input_file_ids)
                self.assertEqual(set([f.name for f in input_files]), set(
                    ["mac", "mac2"]))  # Check input file names

                owned_output_file_ids = set(
                    t.output_file_ids).intersection(t.owned_file_ids)
                self.assertEqual(len(owned_output_file_ids),
                                1)  # 1 output file is owned
                owned_output_files = proj.get_files(id=owned_output_file_ids)
                self.assertEqual(owned_output_files[0].name, "task_result")

                inherited_output_file_ids = set(
                    t.output_file_ids).intersection(t.inherited_file_ids)
                # 5 output files are inherited
                self.assertEqual(len(inherited_output_file_ids), 5)
                inherited_output_files = proj.get_files(
                    id=inherited_output_file_ids)
                self.assertEqual(set([f.name for f in inherited_output_files]), set(
                    ["out", "img", "err", "console_output"]))  # Check output file names

                # Find the task output file and compare the contained variable task_var with task id
                intersection = set(t.output_file_ids).intersection(
                    t.owned_file_ids)
