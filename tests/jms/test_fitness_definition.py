...
class FitnessDefitionTest(REPTestCase):
    ...
    def test_fitness_definition_deserialization(self):
        ...
        for i, ftd in enumerate(ftds):
            ...
            self.assertEqual(ftd.weighting_factor, fd_dict["fitness_term_definitions"][i]["weighting_factor"])
    ...
    def test_fitness_definition_integration(self):
        ...
        job_def.fitness_definition.add_fitness_term(  name="test_ftd", type="design_objective", expression="0.0", weighting_factor=1.0 )
        ...
        self.assertEqual(job_def.fitness_definition.fitness_term_definitions[0].type,"design_objective")
        self.assertEqual(job_def.fitness_definition.fitness_term_definitions[0].expression, "0.0")
        
        job_def.fitness_definition.add_fitness_term(  name="another_term", type="target_constraint", weighting_factor=0.2, expression="2.0" )
        job_def = proj.update_job_definitions([job_def])[0]
        self.assertEqual(len(job_def.fitness_definition.fitness_term_definitions), 2)
        self.assertEqual(job_def.fitness_definition.fitness_term_definitions[1].name, "another_term")
        self.assertEqual(job_def.fitness_definition.fitness_term_definitions[1].type, "target_constraint")
        self.assertEqual(job_def.fitness_definition.fitness_term_definitions[1].expression, "2.0")
        self.assertEqual(job_def.fitness_definition.fitness_term_definitions[1].weighting_factor, 0.2)
